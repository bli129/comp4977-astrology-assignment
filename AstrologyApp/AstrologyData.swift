...
class DataModel: NSObject {
    ...
    func zodiacSign(for month: Int, day: Int) -> AstrologyData? {
        // Loop through each astrology sign
        for sign in DataModel.data {
            // Safely unwrap dateStart and dateEnd values
            if let startMonth = sign.dateStart.month,
               let startDay = sign.dateStart.day,
               let endMonth = sign.dateEnd.month,
               let endDay = sign.dateEnd.day {
        ...
        return nil // Return nil if no match
    }
}
